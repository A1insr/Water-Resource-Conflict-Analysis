...
def default_water_security_yazd_params() -> WaterSecurityYazdParams:
    ...
    gov_factor = {
        ...
        "R_G": 0.9,  # reallocation reduces Yazd's share
    }
...
# ---------------------------------------------------------------------
# 8.5) Urban + Industrial water security (Yazd) — merged criterion (Option A)
# ---------------------------------------------------------------------
@dataclass
class UrbanIndustrialWaterSecurityYazdParams:
    """
    Merged Yazd criterion combining:
      - Drinking/urban water security (supply-to-demand ratio)
      - Industrial water sustainability (supply-to-need ratio)

    The merged raw index is a weighted sum:

        UI_raw = w_drink * U1_raw + w_ind * U2_raw

    where:
      - U1_raw = (effective_drinking_supply / drinking_demand)
      - U2_raw = (effective_industrial_supply / industrial_need)

    Rationale
    ---------
    This prevents double counting (drinking vs industry) while preserving Yazd's priority
    for drinking water (default: 0.70) over industry (default: 0.30).

    Strategy effects are intentionally "softened" relative to the original separate criteria
    to reduce automatic dominance, as agreed by the author:
      - Yazd A_Y: 1.10 → 1.05 (drinking factor), and +0.10 → +0.06 (industry delta)
      - Gov R_G: 0.85 → 0.90 (drinking factor), and -0.12 → -0.10 (industry delta)
      - Isfahan disruption effects slightly softened.
    """
    # Internal weights
    w_drink: float = 0.70
    w_ind: float = 0.30

    # Drinking anchors (same as original drinking-water security criterion)
    population_served: float = 1_470_000
    per_capita_lpd: float = 159.0
    annual_supply_mcm_base: float = 145.0

    # Drinking multiplicative factors
    yazd_factor: Dict[YazdAction, float] = None
    gov_factor: Dict[GovPolicy, float] = None
    isf_factor: Dict[IsfAction, float] = None
    precip_factor: Dict[HydroState, float] = None

    # Industry anchors (same as original industry criterion)
    supply0_ind_mcm: float = 65.0
    need_ind_mcm: float = 90.0

    # Industry fractional deltas on supply0
    yazd_delta: Dict[YazdAction, float] = None
    gov_delta: Dict[GovPolicy, float] = None
    isf_delta: Dict[IsfAction, float] = None


def default_urban_industrial_water_security_yazd_params() -> UrbanIndustrialWaterSecurityYazdParams:
    """Default params for the merged Yazd water-security criterion (Option A)."""
    p = UrbanIndustrialWaterSecurityYazdParams()

    # Drinking (softened)
    p.yazd_factor = {"S_Y": 1.00, "A_Y": 1.05}
    p.gov_factor = {"Q_G": 1.00, "C_G": 1.00, "R_G": 0.90}
    p.isf_factor = {"S_I": 1.00, "M_I": 0.96, "D_I": 0.92}
    p.precip_factor = {"dry": 0.95, "normal": 1.00, "wet": 1.05}

    # Industry (softened)
    p.yazd_delta = {"S_Y": 0.00, "A_Y": 0.06}
    p.gov_delta = {"Q_G": 0.00, "C_G": 0.00, "R_G": -0.10}
    p.isf_delta = {"S_I": 0.00, "M_I": -0.08, "D_I": -0.12}

    return p


def _annual_demand_mcm_from_pop_lpd(population_served: float, per_capita_lpd: float) -> float:
    """Liters/day -> million m^3/year"""
    return float(population_served * (per_capita_lpd / 1000.0) * 365.0 / 1_000_000.0)


def urban_industrial_water_security_index_yazd(
        params: UrbanIndustrialWaterSecurityYazdParams,
        gov_policy: GovPolicy,
        isf_action: IsfAction,
        yazd_action: YazdAction,
        *,
    ...
    """
    Compute merged Yazd (urban+industrial) water-security raw index (NOT normalized).
    ...
    """
    # U1: drinking/urban
    demand_mcm = _annual_demand_mcm_from_pop_lpd(params.population_served, params.per_capita_lpd)
    supply_drink = (
        params.annual_supply_mcm_base
        * params.gov_factor.get(gov_policy, 1.0)
        * params.isf_factor.get(isf_action, 1.0)
    ...
    u1 = 0.0 if demand_mcm <= 0 else float(supply_drink / demand_mcm)

    # U2: industry
    need = float(params.need_ind_mcm)
    delta = (
        params.yazd_delta.get(yazd_action, 0.0)
        + params.gov_delta.get(gov_policy, 0.0)
        + params.isf_delta.get(isf_action, 0.0)
    )
    supply_ind = max(0.0, float(params.supply0_ind_mcm) * (1.0 + float(delta)))
    u2 = 0.0 if need <= 0 else float(supply_ind / need)

    w1 = float(params.w_drink)
    w2 = float(params.w_ind)
    # Defensive normalize weights if needed
    if w1 < 0: w1 = 0.0
    if w2 < 0: w2 = 0.0
    s = w1 + w2
    if s <= 0:
    ...
    return float(w1 * u1 + w2 * u2)


def build_urban_industrial_water_security_scores_yazd_for_scenarios(
        params: UrbanIndustrialWaterSecurityYazdParams,
        *,
        hydro_state: HydroState = "normal",
) -> Dict[str, float]:
    """Scenario scores for merged Yazd urban+industrial water security in [0,100]."""
    rows = []
    keys = []
    ...
    for I in I_STRATS:
        for G in G_STRATS:
            for Y in Y_STRATS:
                scen = f"{I}_{G}_{Y}"
                idx = urban_industrial_water_security_index_yazd(
                    params,
                    gov_policy=G,
                    isf_action=I,
                    yazd_action=Y,
...
# ---------------------------------------------------------------------
# 8.7) Conflict escalation cost (Yazd) — new countervailing criterion (Option A)
# ---------------------------------------------------------------------
@dataclass
class ConflictEscalationCostYazdParams:
    """
    Countervailing Yazd criterion to reduce one-directional incentives.

    This is a *cost* indicator (higher = worse for Yazd), representing internal and external
    costs of conflict escalation (e.g., political/security/legal/operational disruption).

    Raw cost index (0..10 scale) is modeled additively:

        K = base + ΔY(s_Y) + ΔI(s_I) + ΔG(s_G) + interaction

    Final scenario scores are returned as benefit scores in [0,100]:
        score = 100 - minmax(K)
    """
    base: float = 0.0
    yazd_cost: Dict[YazdAction, float] = None
    isf_cost: Dict[IsfAction, float] = None
    gov_cost: Dict[GovPolicy, float] = None

    interaction_cost: float = 1.5  # extra cost if A_Y and (M_I or D_I)


def default_conflict_escalation_cost_yazd_params() -> ConflictEscalationCostYazdParams:
    """Default params for Yazd conflict-escalation cost (Option A), per author agreement."""
    p = ConflictEscalationCostYazdParams()
    p.yazd_cost = {"S_Y": 0.0, "A_Y": 3.0}
    p.isf_cost = {"S_I": 0.0, "M_I": 2.0, "D_I": 5.0}
    p.gov_cost = {"Q_G": 1.5, "C_G": 0.5, "R_G": 2.0}
    p.interaction_cost = 1.5
    return p


def conflict_escalation_cost_index_yazd(
        params: ConflictEscalationCostYazdParams,
        gov_policy: GovPolicy,
        isf_action: IsfAction,
        yazd_action: YazdAction,
) -> float:
    """Raw cost K (higher = worse)."""
    k = (
        float(params.base)
        + float(params.yazd_cost.get(yazd_action, 0.0))
        + float(params.isf_cost.get(isf_action, 0.0))
        + float(params.gov_cost.get(gov_policy, 0.0))
    )
    if (yazd_action == "A_Y") and (isf_action in ("M_I", "D_I")):
        k += float(params.interaction_cost)
    return float(k)


def build_conflict_escalation_cost_scores_yazd_for_scenarios(
        params: ConflictEscalationCostYazdParams,
) -> Dict[str, float]:
    """
    Scenario scores for Yazd escalation cost as a benefit score in [0,100]
    ...
    for I in I_STRATS:
        for G in G_STRATS:
            for Y in Y_STRATS:
                scen = f"{I}_{G}_{Y}"
                k = conflict_escalation_cost_index_yazd(params, G, I, Y)
                keys.append(scen)
                rows.append(k)

    k_arr = np.array(rows, dtype=float)
    score_arr = _min_max_normalize_inverse(k_arr)
    return {keys[i]: float(score_arr[i]) for i in range(len(keys))}
